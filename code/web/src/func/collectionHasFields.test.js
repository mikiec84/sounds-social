import { collectionHasFields, collectionHasLabelAndValue } from './collectionHasFields'
...
test('collectionHasFields', () => {
  ...
  expect(collectionHasFields(['first', 'sec'])([
    { first: true, sec: false },
  ])).toBe(true)

  expect(collectionHasFields(['first', 'sec'])([
    { firsta: true, sec: false },
  ])).toBe(false)
})
